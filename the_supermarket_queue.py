...
def queue_time(customers: list, n: int):
    if len(customers) <= 0:
        return 0
    elif 0 < len(customers) <= n:
        return max(customers)
    else:
        queues = [0 for i in range(n)]
        for next_man in customers:
            # heapify(queues)
            # queues[0] += next_man
            heapreplace(queues, queues[0] + next_man)
        print(queues)
        return max(queues)


print(queue_time([2, 2, 3, 3, 6, 4], 7))
print(queue_time([1, 2, 3, 4, 5], 100))
print(queue_time([1, 22, 13, 24, 35, 6, 74, 28, 9, 10], 5))
print(queue_time([], 1))
